...
- `update` は `cat-self-update-lib` を使って self update を起動します
- `check` は現在のビルドとGitHub上のmainブランチを比較します
...
- **Delete**: カーソル位置の文字を削除
- **F2**: MMLテンプレートを順番に切り替え
- **Ctrl+P**: 再生モードを切り替え（現在の音のみ ⇔ MML全体）
- **ESC**: エディタを終了
...
- 再生には[cat-play-mml](https://github.com/cat2151/cat-play-mml)が子プロセスとして使用されます
  - cat-play-mmlがインストールされていない場合、初回のMMLノート入力時に自動的にGitHubからインストールされます
  - 初回インストール時はビルドに約1分ほどかかります
  - インストールはバックグラウンドで実行されるため、エディタは通常通り動作し続けます
